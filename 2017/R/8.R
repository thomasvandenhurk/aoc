vector_with_strings = c("	a dec -511 if x >= -4	",
"	pq inc -45 if cfa == 7	",
"	vby dec 69 if tl < 1	",
"	yg dec 844 if v > -6	",
"	tl inc -756 if u != 9	",
...
vecnames <- NULL
ch<- NULL
for (i in 1:length(vector_with_strings)){
  vector_with_strings[i] <- gsub("\t","",vector_with_strings[i])
  ch[[i]] <- strsplit(vector_with_strings[i], " ")
  vecnames[i] <- ch[[i]][[1]][1]
}
vecnames <- unique(vecnames)
values <- rep(0,length(vecnames))
maxvalue <- 0
for (i in 1:length(vector_with_strings)){
  if(eval(parse(text=paste(values[which(vecnames==ch[[i]][[1]][5])],paste(ch[[i]][[1]][6:7],sep=" ",collapse=" "), sep="", collapse="")))){
    if(ch[[i]][[1]][2]=="inc"){
      values[which(vecnames==ch[[i]][[1]][1])] <- values[which(vecnames==ch[[i]][[1]][1])] + as.numeric(ch[[i]][[1]][3])
    }
    else{
      values[which(vecnames==ch[[i]][[1]][1])] <- values[which(vecnames==ch[[i]][[1]][1])] - as.numeric(ch[[i]][[1]][3])
    }
