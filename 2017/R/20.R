particles=c("	p=<-1724,-1700,5620>, v=<44,-10,-107>, a=<2,6,-9>	",
...
#totacc <- NULL
for (i in 1:length(particles)){
  temp <- strsplit(particles[i],", ")
  acc<-rbind(acc,as.numeric(strsplit(temp[[1]][3],"<|>|,")[[1]][-1]))
  vel<-rbind(vel,as.numeric(strsplit(temp[[1]][2],"<|>|,")[[1]][-1]))
  pos<-rbind(pos,as.numeric(strsplit(temp[[1]][1],"<|>|,")[[1]][-1]))
  #totacc[i] <- sum(abs(acc[[i]]))
}

for (i in 1:1000){
  vel <- vel + acc
  pos <- pos + vel
  vel <- vel[!(duplicated(pos) | duplicated(pos, fromLast = TRUE)), ]
  acc <- acc[!(duplicated(pos) | duplicated(pos, fromLast = TRUE)), ]
  pos <- pos[!(duplicated(pos) | duplicated(pos, fromLast = TRUE)), ]
}
