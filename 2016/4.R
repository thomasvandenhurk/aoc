rooms = c("	aczupnetwp-mfyyj-opalcexpye-977[peyac]	",
...
sectorid<-0
for (i in 1:length(rooms)){
  temp<-strsplit(rooms[i],"\\[")
  rooms[i]<-temp[[1]][1]
  checksum[i]<-temp[[1]][2]
  temp<-strsplit(rooms[i],"-")
  h<-table(sort(strsplit(paste(temp[[1]][-length(temp[[1]])],sep="",collapse=""),"")[[1]],method="radix"))
  lev<-unique(sort(as.vector(h),decreasing=T))
  vec<-NULL
  for (j in 1:length(lev)){
  vec<-c(vec,which(as.vector(h)==lev[j]))
  }
  letalf<-paste(names(h)[vec],sep="",collapse="")
  if(checksum[i]==substring(letalf,1,5)){
    sectorid<-sectorid+as.numeric(temp[[1]][length(temp[[1]])])
  }
}
...
for (i in 1:length(rooms)){
  ...
  checksum[i]<-temp[[1]][2]
  temp<-strsplit(rooms[i],"-")
  if(length((strsplit(strsplit(rooms[i],"-")[[1]][1],""))[[1]])==5){
    roomsspecial <- c(roomsspecial,rooms[i])
    sectorid <- c(sectorid,as.numeric(temp[[1]][length(temp[[1]])]))
  }
}
